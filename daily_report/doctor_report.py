...
def get_first_date_to_target_opt(start_date, end_date, target=20, is_md=False, to_excel=True, brand_id=10000347,
    ...
    sql = f"""
            ...
            """
    df = fetch_data(sql, is_md=is_md,key='test_config')
...
def get_detail_info(doctor_ids, start_date, end_date, brand_id=10000347, is_md=False):
    ...
    order by HCP姓名, 成功报道时间 desc
        """
    detail_df = fetch_data(sql, is_md=is_md,key='test_config')
...
@click.command()
@click.option('--start_date', help='开始日期', default='2021-04-19 08:00:00')
@click.option('--end_date', help='结束日期', default='2021-04-19 18:00:00')
@click.option('--is_md', help='是否明德', default=False)
@click.option('--target', help='阈值', default=20)
